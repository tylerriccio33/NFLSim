...
Situation <-
  function(samples,
    ...
    i <- collapse::whichv(samples$down, value = d_down)
    samples <- collapse::ss(samples, i)
    ...
    while (loop_iter < max_iter) {
      ...
      ydstogo_between <-
        between_cpp_parallel(avail_data$ydstogo, cur_lower_ydstogo, cur_upper_ydstogo)
      yard_slice <- collapse::ss(avail_data, ydstogo_between)
      if (collapse::fnrow(yard_slice) == 0) {
      ...
      wp_between <-
        between_cpp_parallel(yard_slice$wp, cur_lower_wp, cur_upper_wp)
      wp_slice <- collapse::ss(yard_slice, wp_between)
      if (collapse::fnrow(wp_slice) == 0) {
      ...
      yardline_between <-
        between_cpp_parallel(wp_slice$yardline_100,
                    cur_lower_yardline,
                    cur_upper_yardline)
      yardline_slice <- collapse::ss(wp_slice, yardline_between)
      if (collapse::fnrow(yardline_slice) == 0) {
      ...
      if (collapse::fnrow(yardline_slice) <= 3 & loop_iter <= max_iter) {
        YDSTOGO_WINDOW <- YDSTOGO_WINDOW + 1
        WP_WINDOW <- WP_WINDOW + .05
        YARDLINE_100_WINDOW <- YARDLINE_100_WINDOW + 5
        loop_iter <- loop_iter + 1
